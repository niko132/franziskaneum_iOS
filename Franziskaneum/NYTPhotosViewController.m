...
- (void)updateOverlayInformation {
    ...
    if (!overlayTitle) {
        ...
        if (self.dataSource.numberOfPhotos > 1) {
            overlayTitle = [NSString localizedStringWithFormat:NSLocalizedString(@"%lu von %lu", nil), (unsigned long)displayIndex, (unsigned long)self.dataSource.numberOfPhotos];
        }
    }
